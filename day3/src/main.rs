...
fn main() {
    let inputs = fs::read_to_string("/home/dragut/Projects/advent-of-code-2021/day3/inputs/input.txt").expect("msg");
    ...
    for binary in binaries {
        let digits: Vec<i32> = binary.split("").filter(|d| d.len() == 1).map(|d| d.parse::<i32>().unwrap()).collect();
    ...
    println!("{:?} {:?} {}", one_freqs, zero_freqs, gamma_rate*epsilon_rate)
}